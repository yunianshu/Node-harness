...
export const DE_AI_WRITING_RULES = [
  ...
  '【节奏落差硬约束】句长必须大开大合：短句群（十字以内连用不超过四句）之后必须接一个四十字以上的长句蓄力，全章长句（四十字以上）不少于六处；禁止通篇均匀的短句或均匀的中句。',
  '【段落落差硬约束】段落长短必须悬殊：单行对话段与三到五行的叙述段交错出现，禁止连续多段行数相同；至少有两处连续短段之后接一个长段。',
  '【用词不可预测】优先用具体的名词与动词，不用概括性雅词（如「复杂的情绪」「某种意味」）；同一修辞手法不连续复用。',
]
