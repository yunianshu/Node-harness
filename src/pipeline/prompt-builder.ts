...
export const DE_AI_WRITING_RULES = [
  ...
  '对话一律用弯引号（如 "别问。"），禁用「」直角引号。',
]

export const DE_AI_OUTLINE_RULES = [
  '章纲的每个场景必须有目标、有动作、有变化。',
  '场景计划禁止原地转圈：相邻场景必须带来新信息或新后果。',
  '场景计划避免每场都「悬念→反转→收束」的完美闭环：允许平淡过场、冲突无结果地散去、悬念悬而不决；章节不必每章都收在悬念钩子上，允许平淡收场。',
  '摘要用白话，禁止翻案句式与报告腔。',
  '【线索节制】配角不必人人服务于本章主谜题：允许有人提供无关信息、说错话、话到嘴边咽回去、线索不完整或互相矛盾。世界独立于主角而存在，不是为谜题搭的布景。',
  '【功能型人物规避】每个出场的配角都带一点与主谜题无关的自身生活（怪癖、习惯、私事、来历）。禁止「神秘人出现→说谜语→消失」模板；若需神秘角色，至少给他一个不神秘的具体细节（口吃、鞋底破、说完被风呛之类）。',
]
